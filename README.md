...
For the OpenMP GEM5 port, the `-s` argument is the total aggregate array size for the whole process.
The runtime divides that total into contiguous per-thread slices.
...
These are notable differences from the original STREAM benchmark:
 ...
 - In the GEM5 OpenMP path, the kernel is repeated a fixed number of times to create a bounded ROI, similar to STREAM timing loops.
...
The assembly kernels advance in fixed 1600-element chunks.
If you want the requested `-s` value to match the exact touched range, choose a multiple of 1600 elements.
